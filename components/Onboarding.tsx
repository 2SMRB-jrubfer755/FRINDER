...
const Onboarding: React.FC<OnboardingProps> = ({ onComplete, t, onBack }) => {
  ...
  const renderStep = () => {
    switch (step) {
      case 1: // Form Info
        return (
          <div className="space-y-6 md:space-y-8 animate-in slide-in-from-right duration-500 flex flex-col items-center w-full">
            <h2 className="text-2xl sm:text-3xl md:text-4xl font-black text-center text-accent uppercase tracking-tight italic">{isLoginMode ? 'BIENVENIDO DE VUELTA' : 'NUEVO JUGADOR'}</h2>
            <div className="w-full space-y-4 md:space-y-5">
              {!isLoginMode && (
                <input 
                  type="text" 
                  placeholder="Nombre de Gamer" 
                  className="w-full glass bg-transparent border-b-2 border-accent/20 p-3.5 md:p-4 text-base md:text-lg outline-none focus:border-primary transition-all text-center placeholder:text-accent/20"
                  value={formData.name}
      ...
      case 2: // Avatar Selection
        return (
          <div className="space-y-6 md:space-y-8 animate-in slide-in-from-right duration-500 w-full">
            <h2 className="text-2xl md:text-3xl font-black text-center text-accent uppercase tracking-[0.14em] md:tracking-[0.2em] flex items-center justify-center gap-2 italic">
               <span>📸</span> TU IMAGEN
            </h2>
            <div className="grid grid-cols-3 gap-4 md:gap-6">
                {avatars.map(url => (
                    <button 
                        ...
                        className={`aspect-square rounded-2xl md:rounded-[28px] overflow-hidden border-4 transition-all hover:scale-105 ${formData.avatar === url ? 'border-primary shadow-[0_0_30px_rgba(161,24,24,0.5)] scale-105' : 'border-transparent opacity-40 hover:opacity-100'}`}
                    >
            ...
            <div className="flex gap-3">
                ...
                <button onClick={handleNext} className="flex-[2] py-3.5 md:py-4 bg-accent text-secondary font-black text-sm md:text-base rounded-2xl shadow-2xl active:scale-95 transition-all">CONTINUAR</button>
            </div>
          </div>
        );
      case 3: // Tastes and Skills
        return (
          <div className="space-y-6 md:space-y-8 animate-in slide-in-from-right duration-500 w-full">
            ...
            <div className="flex gap-3">
                <button onClick={handleBack} className="flex-1 py-3.5 md:py-4 glass border border-accent/20 text-accent font-black text-sm md:text-base rounded-2xl active:scale-95 transition-all">ATRÁS</button>
                <button onClick={handleNext} className="flex-[2] py-3.5 md:py-4 bg-accent text-secondary font-black text-sm md:text-base rounded-2xl shadow-2xl active:scale-95 transition-all">¡BUENA PINTA!</button>
            </div>
          </div>
        );
        ...
        return (
          <div className="space-y-6 md:space-y-8 animate-in slide-in-from-right duration-500 w-full">
            ...
            <div className="space-y-5 md:space-y-7">
              <div className="glass p-5 md:p-7 rounded-2xl md:rounded-3xl border border-white/5 shadow-xl">
                <div className="flex justify-between items-center mb-6">
                  <p className="text-xs font-black text-accent/50 uppercase tracking-[0.3em]">Distancia Máxima 📍</p>
                  <span className="text-xl md:text-2xl font-black text-accent">{formData.distance} <span className="text-xs opacity-50">KM</span></span>
                </div>
                <input 
              ...
              <div className="glass p-5 md:p-7 rounded-2xl md:rounded-3xl border border-white/5 shadow-xl">
                <div className="flex justify-between items-center mb-6">
                  ...
                  <span className="text-xl md:text-2xl font-black text-accent">{formData.minAge}-{formData.maxAge} <span className="text-xs opacity-50">AÑOS</span></span>
                </div>
                <div className="flex gap-3 md:gap-5">
                  <div className="flex-1 space-y-2">
                    ...
                    <input 
                      type="number" 
                      className="w-full glass p-3.5 md:p-4 text-center text-lg md:text-xl font-black rounded-xl md:rounded-2xl border-2 border-white/5 outline-none focus:border-primary transition-all" 
  ...
  return (
    <div className="h-screen w-screen bg-secondary flex items-center justify-center p-4 sm:p-6 overflow-y-auto">
      <div className="max-w-xl w-full py-6 md:py-8 flex flex-col items-center">
        {renderStep()}
      </div>
    </div>
  );
