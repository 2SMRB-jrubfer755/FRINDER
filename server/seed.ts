...
import User, { Gender, GameType } from './models/User';
...
const seedDB = async () => {
    try {
        ...
        // Update groups with some members
        MOCK_GROUPS[0].members = [users[0]._id.toString(), users[2]._id.toString(), users[4]._id.toString()];
        MOCK_GROUPS[1].members = [users[1]._id.toString(), users[3]._id.toString()];

        const groups = await Group.insertMany(MOCK_GROUPS);
        console.log(`Seeded ${groups.length} groups`);

        const tournaments = await Tournament.insertMany(MOCK_TOURNAMENTS);
